...
def extract_data(tif_path,shp_path,field=[],tif_name=''):
    ...
    for index, feature in enumerate(layer):
        ...
        if raster_block_filtered.shape[0]!=0:
            ...
            if len(fields)!=0:
                fields_mas=numpy.empty((raster_block_filtered.shape[0],len(fields)))
                if len(fields)==1:
                    result_fields=numpy.concatenate([result_fields,[feature.GetField(fields[0])]],axis=0)
                else:
                    for i in range(len(fields)):
                        ff=feature.GetField(fields[i])
                        fields_mas[0:raster_block_filtered.shape[0],i:i+1]=field_paste(ff,raster_block_filtered.shape[0])
                    result_fields=numpy.concatenate([result_fields,fields_mas],axis=0)
    src_shapefile = None
    sys.stdout.write('/n')
    if len(fields)!=0:
...
def classify(path,i0,clf, name='',prob=0,tif_name=''):
    ...
    if path.find('s3://')==-1:
        if len(name)==0:
            output = driver.Create(os.path.join(path,'classification_map.tif'),tif_param.xsize,tif_param.ysize,1+prob,gdal.GDT_Byte)
        else:
            output = driver.Create(os.path.join(path,name),tif_param.xsize,tif_param.ysize,1+prob,gdal.GDT_Byte)
...
if __name__ == "__main__":
    ...
    time_0 = time.time()
    classify(folder_path, 0, clf, tif_name='.tif')
    print(int(time.time() - time_0))
